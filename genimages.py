...
def _ambil_gambar(data: dict) -> str:
    ...
    Bentuk respons Interactions API bisa berbeda antar versi, jadi kita
    telusuri semua bentuk yang pernah didokumentasikan daripada mengandalkan
    satu jalur saja.
    """
    # Bentuk 1: steps[].content[] dengan type "image"
    for step in data.get("steps", []) or []:
        for c in step.get("content", []) or []:
            if c.get("type") == "image" and c.get("data"):
                return c["data"]
    # Bentuk 2: interaction.output_image.data
    oi = (data.get("interaction", {}) or {}).get("output_image") or {}
    if oi.get("data"):
        return oi["data"]
    if (data.get("output_image") or {}).get("data"):
        return data["output_image"]["data"]
    # Bentuk 3: generateContent lama — candidates[].content.parts[].inlineData
...
def gen_gemini(prompt: str, aspect: str, model: str) -> bytes:
    ...
    # Dicoba dari yang paling lengkap ke yang paling sederhana. Bentuk
    # terakhir persis seperti contoh resmi di dokumentasi, jadi kalau semua
    # gagal, masalahnya bukan di bentuk permintaan.
    varian = [
    ...
    for body in varian:
        ...
        if r.ok:
            ...
            continue
        # Tampilkan penjelasan dari Google, bukan cuma kode statusnya.
        try:
            pesan = r.json().get("error", {}).get("message", r.text)[:220]
        except Exception:  # noqa: BLE001
            pesan = r.text[:220]
        galat.append(f"{r.status_code}: {pesan}")
        # 401/403 = kunci atau izin bermasalah; mencoba varian lain sia-sia.
...
def process(path: pathlib.Path, force: bool) -> int:
    ...
    for spec in imgs:
        ...
        for attempt in range(1, 4):
            try:
                # Google bisa mengembalikan JPEG atau PNG tergantung varian
                # permintaan yang diterima. Kita simpan selalu sebagai PNG
                # supaya nama file yang dirujuk content.json tetap cocok.
